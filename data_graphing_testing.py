...
def generate_figure(df,
    ...
    fig, axs = plt.subplots(ncols=len(condition_list), sharey=True, sharex=True)
    ...
    for ix, ax in enumerate(axs):
        if ix == 0:
            ...
            ax.set_xlabel(condition_list[ix], fontsize=30, weight='bold')
            ax.set_yticks([round(i, 2) for i in ax.get_yticks()])
            ax.set_yticklabels([str(round(i, 2)) for i in ax.get_yticks()], fontsize=0, weight='bold')
